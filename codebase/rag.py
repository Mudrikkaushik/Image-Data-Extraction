...
text_file_path = "/teamspace/studios/this_studio/Result/invoice_data.txt"
...
def run_analysis():
    ...
    try:
        result = subprocess.run(
            ['python3', '/teamspace/studios/this_studio/Main/analysis.py'],
            check=True,
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE
        )
        st.success("Invoice data extracted and saved to text file.")
        st.text_area("Analysis Output", result.stdout.decode(), height=200)
    except subprocess.CalledProcessError as e:
        st.error(f"Analysis failed: {e.stderr.decode()}")

def load_invoice_data_from_txt(file_path):
    """Load invoice data from text file."""
...
def main():
    ...
    if os.path.exists(text_file_path):
        ...
        # Load the invoice data from the text file
        data = load_invoice_data_from_txt(text_file_path)
        ...
        if st.button("Run Query"):
            if prompt_text:
                with st.spinner("Processing your query..."):
                    ...
                    response = query_ollama(prompt_text, model=selected_model, context=data)
        ...
        st.warning(f"Text file not found at {text_file_path}. Please run the analysis first.")
